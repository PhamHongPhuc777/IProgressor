...
> Converted from `Project Requirement Document - Project Managment Web App.pdf` for repo review/reference. Content is unedited from the source PDF (only reformatted to Markdown), **except for the corrections/additions noted below.**
...
- [ ] **[M]** Đính kèm tài liệu, liên kết file từ Google Drive
...
|-----------------------|-------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Frontend              | React.ts (self-host)          | SPA thuần, vì đây là tool nội bộ, có xác thực — Vite cho dev/build ổn định hơn tự dùng bundler của Bun                                                                                                                                             |
| Form & State          | React Hook Form + Zustand     | Chuẩn phổ biến của hệ sinh thái React, tương thích hoàn toàn                                                                                                                                                                                       |
| Styling               | TailwindCSS + ShadCN          | Toàn quyền custom, không nặng budle size, Type-safe thuần hỗ trợ cho ngôn ngữ TypeScript và tốc độ rất nhanh                                                                                                                                       |
| Runtime environment   | Bun.js                        | Dùng làm package manager & chạy script (bun install, bun run) để tăng tốc phát triển UI                                                                                                                                                            |
| Backend               | Spring Boot + Spring Modulith | Modular monolith (tổ chức package theo module: workspace, project, task, auth...)                                                                                                                                                                  |
| Bảo mật tầng API      | Spring Security               | Xác thực JWT do Zitadel phát hành cho mọi request; enforce RBAC theo workspace ngay tại API, không chỉ ở giao diện                                                                                                                                 |
| Build tool            | Gradle                        | Daemon + incremental/cache build nhanh hơn Maven, DSL Groovy linh hoạt hơn XML                                                                                                                                                                     |
| Database              | PostgreSQL (cloud)            | Host trên dịch bụ bên thứ ba, kiểm soát và giám sát hoàn toàn dữ liệu trên Supabase                                                                                                                                                                |
...
| VPN nội bộ            | NetBird (cloud)               | Bảo vệ các cổng nhạy cảm, kết nối DB và WebUI URL trực tiếp — NetBird có policy/ACL theo nhóm (sync trực tiếp từ Zitadel qua OIDC) xuống tới từng host/service cụ thể                                                                              |
| OIDC bên ngoài        | Google (OAuth2)               | Một lựa chọn "Đăng nhập bằng Google" bổ sung cho Zitadel — cấu hình tại Zitadel                                                                                                                                                                    |
| Tích hợp chia sẻ file | Google Drive API              | Đăng ký app/API key qua Google Cloud Console để cung cấp tính năng chia sẻ song sử dụng file chung                                                                                                                                                 |
| Email/SMTP            | Brevo                         | Zitadel tự gửi email vòng đời tài khoản (mã xác minh, đặt lại mật khẩu, email mời khởi tạo sau khi duyệt truy cập) qua SMTP                                                                                                                        |

> **Addition note (2026-07-23):** "Tích hợp chia sẻ file" was originally Microsoft Graph API/SharePoint (matching Microsoft 365 infrastructure). Switched to Google Drive API after hitting real Microsoft Entra ID tenant-eligibility friction while setting this up (no free/self-serve tenant available without a qualifying Microsoft 365 subscription or nonprofit/education affiliation) -- see `markdown/SETUP.md`'s "Google Drive" section for the setup this replaced.
