...
4. [Yêu cầu phi chức năng](#4-yêu-cầu-phi-chức-năng)
   ...
   - [NFR-4: Khả năng triển khai (Deferred)](#nfr-4-khả-năng-triển-khai-deferred)
...
- **Giám đốc (Leader):** theo dõi tổng quan tiến độ và hiệu suất của mọi phòng ban, phát hiện rủi ro trễ tiến độ, gửi thông báo toàn hệ thống.
- **Quản trị hệ thống (Admin):** quản lý toàn hệ thống, tích hợp, bảo mật, thành viên và phân quyền.
...
- Hiển thị dữ liệu theo nhiều dạng view: Kanban board, danh sách, Gantt chart, lịch. *(Việc này áp dụng cho mọi vai trò — Staff/PM/Leader/Admin đều xem được cả 4 kiểu view; mục này được liệt kê dưới FR-3 vì đây là nơi PM chủ động lập kế hoạch dùng các view đó, không phải vì view bị giới hạn riêng cho PM.)*

### FR-4: Theo dõi và báo cáo cho vai trò giám đốc

- Dashboard tổng quan tiến độ dự án theo workspace/phòng ban.
- Báo cáo khối lượng công việc (workload) và hiệu suất theo từng nhân sự. *(Theo dõi workload/phân bổ nguồn lực thuộc phạm vi của PM — mỗi PM xem workload trong phòng ban mình; tổng hợp hiệu suất & rủi ro trên toàn bộ các phòng ban thuộc phạm vi của Giám đốc — xem chi tiết trong Functional Requirements table và UI mapping doc.)*
...
### NFR-4: Khả năng triển khai (Deferred)

**Trạng thái: chưa đặc tả — có chủ đích, không phải thiếu sót.**

Dự án hiện đang ở giai đoạn **local development**; hạ tầng production (Zitadel/NetBird self-host, TLS/domain, CI/CD, chiến lược scale, backup/restore) chưa được cấu hình hay quyết định. Mục này sẽ được viết đầy đủ khi dự án chuyển sang giai đoạn chuẩn bị production, dự kiến bao gồm:
...
Không cần hành động gì cho phần này ở giai đoạn hiện tại — chỉ cần quay lại mục này trước khi lên kế hoạch go-live.
